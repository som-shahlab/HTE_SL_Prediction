...
HTEcalib <- function(data, times, g=5, name="", calib_plot=T){
  ...
  # #             treatment effects. J Clin Epidemiol. 2018 Feb;94:59-68.
  # tmp$obs_benefit <- ifelse(tmp$D1==1 & tmp$Y1<tmp$Y0, 1,
  #                           ifelse(tmp$D0==1 & tmp$Y1>tmp$Y0, -1, 0))
  # 
  # tmp$avg_benefit <- (tmp$pred_benefit0 + tmp$pred_benefit1)/2
  # tmp <- tmp%>%filter(!is.na(obs_benefit))
  # tmp$cut.delta <- cut(tmp$avg_benefit, breaks = unique(quantile(tmp$avg_benefit, probs=seq(0,1,1/g))), include.lowest=TRUE)
  # 
  # # observed ASD
  # obs.delta <- aggregate(tmp$obs_benefit, by = list(tmp$cut.delta), FUN = "mean")$x
  # 
  # # expected ARD
  # exp.delta <- aggregate(tmp$avg_benefit, by = list(tmp$cut.delta), FUN = "mean")$x
  
  #--------------------------------- Another way -----------------------------------------------#
  data$cut.delta <- cut(data$pred_benefit, breaks = unique(quantile(data$pred_benefit, probs=seq(0,1,1/g))), include.lowest=TRUE)
  exp.delta  <- aggregate(data$pred_benefit, by = list(data$cut.delta), FUN = "mean")$x

  # bootstrap-sampling observed ARR
  obsf <- function(data,times,indicies){
    d <- data[indicies,]
  ...
  boot_obs <- boot(data = data, statistic = obsf, R=1000, times=times)
  obs.delta <- boot_obs$t0
  CI_obs <- matrix(NA,g,2)
  for (k in 1:g){
    CI_obs[k,] <- boot.ci(boot_obs, conf=0.95, type="perc", index=k)$percent[4:5]
  }
  
  HTEests <- data.frame(obs_delta = obs.delta, 
                        exp_delta = exp.delta,
                        LB_obs = CI_obs[,1],
                        UB_obs = CI_obs[,2])
  ...
  if(calib_plot==T){
    ...
    p <- ggplot(HTEests, aes(x = exp.delta, y = obs.delta)) +
      theme_classic()+
