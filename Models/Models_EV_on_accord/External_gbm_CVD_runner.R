...
accord_cvdtr1 <- accord_cvd[accord_cvd$INTENSIVE==1,c(1:2,4:dim(accord_cvd)[2])]
accord_cvdtr0 <- accord_cvd[accord_cvd$INTENSIVE==0,c(1:2,4:dim(accord_cvd)[2])]
...
for (l in 1:length(maxdepths)){
  ...
  for (j in 1:length(minobs)){
    ...
    for (k in 1:length(subsamples)){
      ...
      gbm_tune_treated <- gbm(Surv(t_cvds,cvd)~., data=sprint_cvd1,
                              distribution = "coxph",
                              shrinkage = 0.005,
                              n.trees = 3000,
...
set.seed(1)
gbm_final_treated <- gbm(Surv(t_cvds,cvd)~., data=sprint_cvd1,
                         distribution = "coxph",
                         shrinkage = 0.001,
                         n.trees = 10000,
...
#pred.train1 <- predict(gbm_final_treated, sprint_cvd1, n.trees = best_params_treated$best_iter)
pred.test1  <- predict(gbm_final_treated, accord_cvd[,4:dim(accord_cvd)[2]], n.trees = best_params_treated$best_iter)
# Estimate the cumulative baseline hazard function using treated patients in ACCORD as the training data
accord.train1 <- predict(gbm_final_treated, accord_cvdtr1[,3:dim(accord_cvdtr1)[2]], n.trees = best_params_treated$best_iter)
basehaz.cum1 <- basehaz.gbm(accord_cvdtr1$t_cvds, accord_cvdtr1$cvd, accord.train1, t.eval = t50, cumulative = T)
...
for (l in 1:length(maxdepths)){
  ...
  for (j in 1:length(minobs)){
    ...
    for (k in 1:length(subsamples)){
      bag.fraction <- subsamples[k]
      gbm_tune_control <- gbm(Surv(t_cvds,cvd)~., data=sprint_cvd0,
                              distribution = "coxph",
                              shrinkage = 0.005,
                              n.trees = 3000,
                              interaction.depth = interaction.depth,
...
set.seed(1)
gbm_final_control <- gbm(Surv(t_cvds,cvd)~., data=sprint_cvd0,
                         distribution = "coxph",
                         shrinkage = 0.001,
                         n.trees = 10000,
...
# return a vector of prediction on n.trees indicating log hazard scale.f(x)
#pred.train0 <- predict(gbm_final_control, sprint_cvd0, n.trees = best_params_control$best_iter)
pred.test0  <- predict(gbm_final_control, accord_cvd[,4:dim(accord_cvd)[2]], n.trees = best_params_control$best_iter)
# Estimate the cumulative baseline hazard function using control patients in ACCORD as the training data
accord.train0 <- predict(gbm_final_control, accord_cvdtr0[,3:dim(accord_cvdtr0)[2]], n.trees = best_params_control$best_iter)
basehaz.cum0 <- basehaz.gbm(accord_cvdtr0$t_cvds, accord_cvdtr0$cvd, accord.train0, t.eval = t50, cumulative = T)


# S(X) at median survival time
surf1 <- surf0 <- rep(NA,dim(accord_cvd)[1])
for (j in 1:dim(accord_cvd)[1]){
...
for (l in 1:length(maxdepths)){
  ...
  for (j in 1:length(minobs)){
    ...
    for (k in 1:length(subsamples)){
      bag.fraction <- subsamples[k]
      gbmfit2 <- gbm(Surv(t_cvds,cvd)~., data=sprint_cvd,
                     distribution = "coxph",
                     shrinkage = 0.005,
                     n.trees = 10000,
                     interaction.depth = interaction.depth,
...
set.seed(1)
gbmfit_final <- gbm(Surv(t_cvds,cvd)~., data=sprint_cvd, 
                    distribution = "coxph",
                    shrinkage = 0.001, 
                    n.trees = 20000,   
...
# return a vector of prediction on n.trees indicating log hazard scale.f(x)
pred.test1 <- predict(gbmfit_final, accord_cvd1[,3:dim(accord_cvd1)[2]], n.trees = best_params_S$best_iter)
pred.test0 <- predict(gbmfit_final, accord_cvd0[,3:dim(accord_cvd0)[2]], n.trees = best_params_S$best_iter)

# Estimate the cumulative baseline hazard function using ACCORD as the training data
accord.train <- predict(gbmfit_final, accord_cvd[,3:dim(accord_cvd)[2]], n.trees = best_params_S$best_iter)
basehaz.cum <- basehaz.gbm(accord_cvd$t_cvds, accord_cvd$cvd, accord.train, t.eval = t50, cumulative = T)

# S(X) at median survival time 
surf1 <- surf0 <- rep(NA,dim(accord_cvd1)[1])
for (j in 1:dim(accord_cvd1)[1]){
...
write.csv(risk_gbm_cvd, paste0("./cvd_tuned_results/external_gbm_cvd.csv"), row.names = F)

best_params <- data.frame(t(best_params_control), t(best_params_treated), t(best_params_S))
write.csv(best_params, paste0("./cvd_tuned_results/external_gbm_best_params_cvd.csv"), row.names = F)
