...
#                              times=train_dat1$t_saes,
#                              delta=train_dat1$sae,
#                              ntree=ntree,
#                              k=sd,
#                              power=2,
#                              base=0.95,
...
#       D1 <- ifelse(valid_dat1$t_saes>t50,0,valid_dat1$sae)
#       NLL1 <- -1*mean((1-D1)*log(surv_est1)+D1*log(1-surv_est1))
#       pet1 <- rbind(pet1, c(ntree, sd, NLL1))
# 
#       # # IBS
#       # PredError1 <- pec(object=bartSurv1,
#       #                   formula = as.formula("Surv(t_saes, sae)~1"),
#       #                   cens.model="marginal",
#       #                   start=min(valid_dat1$t_saes),
#       #                   maxtime = t50,
#       #                   exact = F,
#       #                   exactness = 100,
#       #                   reference = F,
...
#       # pe1 <- crps(PredError1, times=t50, start=min(valid_dat1$t_saes))  # OOB prediction error based on Graf's integrated BS (1999)
#     }
#   }
#   petable1[[m]] <- pet1
# }
...
#                        times=traindat1$t_saes,
#                        delta=traindat1$sae,
#                        events = traindat1$t_saes[traindat1$sae==1],
#                        ntree=opt_params1[[1]],
#                        k=opt_params1[[2]],
#                        power=2,
#                        base=0.95,
...
bartSurv1 <- mc.surv.bart(x.train=traindat1[,3:dim(traindat1)[2]],
                          times=ceiling(traindat1$t_saes/30),
                          delta=traindat1$sae,
                          ntree=50,
                          x.test=testdat[,3:dim(testdat)[2]],
                          seed=99, 
...
# save results 
risk_bart_sae <- data.frame(testIndexes, Tbart_risk_treated_sae)
write.csv(risk_bart_sae, paste0("./sae_tuned_results/stratified_T1bart_sae",i,".csv"), row.names = F)
