...
# S-Learner
x.test.both <- rbind(accord_sae1,accord_sae0)
bartSurv <- mc.surv.bart(x.train=sprint_sae[,3:dim(sprint_sae)[2]],
                         times=ceiling(sprint_sae$t_saes/30),
                         delta=sprint_sae$sae,
                         ntree=50,
                         x.test = x.test.both[,3:dim(accord_sae1)[2]],
                         seed=99, 
                         mc.cores=4)
surv_ests <- t(matrix(bartSurv$surv.test.mean, nrow = length(bartSurv$times)))
surv_est1 <- surv_ests[1:dim(accord_sae1)[1],which.min(abs(sort(bartSurv$times)-t50))]
surv_est0 <- surv_ests[-(1:dim(accord_sae1)[1]),which.min(abs(sort(bartSurv$times)-t50))]
Sbart_risk_treated_sae <- 1-surv_est1
Sbart_risk_control_sae <- 1-surv_est0

# save results 
risk_bart_sae <- data.frame(Sbart_risk_control_sae,Sbart_risk_treated_sae)
write.csv(risk_bart_sae, paste0("./sae_tuned_results/external_Sbart_sae.csv"), row.names = F)
