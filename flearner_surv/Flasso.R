...
Flasso = function(x, tx, y, pscore = rep(.5, nrow(x)), nfolds = NULL, 
                  alpha=NULL, weight, verbose = FALSE) {
...
predict.Flasso = function(object, newx, ...) {
  ...
  return(as.vector(predict(object$tau_fit, newx = newx, s = object$tau_fit$lambda.min)))
