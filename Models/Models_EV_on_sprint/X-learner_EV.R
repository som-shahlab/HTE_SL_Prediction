...
tau1 <- predict(fit1_cvd, accord_test[,vars])$predictions
...
tau0 <- predict(fit0_cvd, accord_test[,vars])$predictions
...
#                                             traindatUC[traindatUC$INTENSIVE==1,vars]),
#                    num.trees = 1000, mtry = 6, min.node.size = 1, regularization.factor = 0.7,
#                    case.weights = traindatUC$sae_ipcw[traindatUC$INTENSIVE==1], importance = "impurity", seed=1)
# tau1 <- predict(fit1_sae, accord_test[,vars])$predictions
...
# tau0 <- predict(fit0_sae, accord_test[,vars])$predictions
# sae0_imp <- fit0_sae$variable.importance
# XL_RD_sae <- (tau1+tau0)/2


# Evaluate 
cvd_S_RD <- data.frame(W = accord_test$INTENSIVE, Y = accord_test$t_cvds, D = accord_test$cvd, pred_benefit = XL_RD_cvd)
CB_Scvd <- cforbenefit(cvd_S_RD);CB_Scvd
calib_Scvd <- HTEcalib(data = cvd_S_RD, times = t50, name = "");calib_Scvd

# sae_S_RD <- data.frame(W = accord_test$INTENSIVE, Y = accord_test$t_saes, D = accord_test$sae, pred_benefit = XL_RD_sae)
# CB_Ssae <- cforbenefit(sae_S_RD);CB_Ssae
# calib_Ssae <- HTEcalib(data = sae_S_RD, times = t50, name = "");calib_Ssae

# Save results
# respath <- "C:/Users/cryst/Documents/Stanford Postdoc/NHLBI R01 Aim 2/Analyses Stanford Team/Analysis Results/External Results/X-learner"
# write.csv(XL_RD_cvd, paste0(respath,"/sf_CVD.csv"), row.names = F)
#write.csv(XL_RD_sae,paste0(respath,"/sf_SAE.csv"), row.names = F)
...
write.csv(sum_imp, paste0(respath,"/EV_imp_sf.csv"), row.names = F)
...
for (k in 1:2){
  ...
  for (p in 1:3){
    ...
    for (q in 1:3){
      ...
      fit1_sae <- ranger(Y ~ ., data = data.frame(Y = traindatUC$sae[traindatUC$INTENSIVE==1] - traindatUC$T_risk_control_sae[traindatUC$INTENSIVE==1],
                                                  traindatUC[traindatUC$INTENSIVE==1,vars]),
                         num.trees = 1000, mtry = mtry, min.node.size = minnode, regularization.factor = fac,
                         case.weights = traindatUC$sae_ipcw[traindatUC$INTENSIVE==1], importance = "impurity", seed=1)
      tau1 <- predict(fit1_sae, accord_test[,vars])$predictions
      sae1_imp <- fit1_sae$variable.importance
      ...
      print(c(mtry,minnode,fac))
      sae_S_RD <- data.frame(W = accord_test$INTENSIVE, Y = accord_test$t_saes, D = accord_test$sae, pred_benefit = XL_RD_sae)
      CB_Ssae <- cforbenefit(sae_S_RD);print(CB_Ssae)
      calib_Ssae <- HTEcalib(data = sae_S_RD, times = t50, name = "");print(calib_Ssae)
    }
  }
}
# ### gbm
...
# 
# set.seed(1)
# fit1_cvd <- gbm(Y ~ ., data = data.frame(Y = traindatUC$cvd[traindatUC$INTENSIVE==1] - traindatUC$T_risk_control_cvd[traindatUC$INTENSIVE==1],
#                                          traindatUC[traindatUC$INTENSIVE==1,vars]),
#                 distribution = "gaussian",
#                 weights = traindatUC$cvd_ipcw[traindatUC$INTENSIVE==1],
#                 shrinkage = 0.01, 
#                 n.trees = 1000,   
#                 interaction.depth = 2, 
#                 n.minobsinnode = 1, 
...
#                 cv.folds = 5,
#                 n.cores = 5) 
# best_iter <- gbm.perf(fit1_cvd, plot.it = T, method = 'cv')
# tau1 <- predict(fit1_cvd, accord_test[,vars], n.trees = best_iter, type="response")
# #cvd1_imp <- fit1_cvd$variable.importance
# 
# fit0_cvd <- gbm(Y ~ ., data = data.frame(Y = traindatUC$T_risk_treated_cvd[traindatUC$INTENSIVE==0] - traindatUC$cvd[traindatUC$INTENSIVE==0],
#                                          traindatUC[traindatUC$INTENSIVE==0,vars]),
#                 distribution = "gaussian",
#                 weights = traindatUC$cvd_ipcw[traindatUC$INTENSIVE==0],
#                 shrinkage = 0.01, 
#                 n.trees = 1000,   
#                 interaction.depth = 2, 
#                 n.minobsinnode = 1, 
...
#                 cv.folds = 5,
#                 n.cores = 5) 
# best_iter <- gbm.perf(fit0_cvd, plot.it = T, method = 'cv')                
# tau0 <- predict(fit0_cvd, accord_test[,vars], n.trees = best_iter, type="response")
# #cvd0_imp <- fit0_cvd$variable.importance
# XL_RD_cvd <- (tau1+tau0)/2
# 
# 
...
# 
# set.seed(1)
# fit1_sae <- gbm(Y ~ ., data = data.frame(Y = traindatUC$sae[traindatUC$INTENSIVE==1] - traindatUC$T_risk_control_sae[traindatUC$INTENSIVE==1],
#                                          traindatUC[traindatUC$INTENSIVE==1,vars]),
#                 distribution = "gaussian",
#                 weights = traindatUC$sae_ipcw[traindatUC$INTENSIVE==1],
#                 shrinkage = 0.01, 
#                 n.trees = 1000,   
#                 interaction.depth = 2, 
#                 n.minobsinnode = 1, 
...
#                 cv.folds = 5,
#                 n.cores = 5) 
# best_iter <- gbm.perf(fit1_sae, plot.it = T, method = 'cv')
# tau1 <- predict(fit1_sae, accord_test[,vars], n.trees = best_iter, type="response")
# #sae1_imp <- fit1_sae$variable.importance
# 
# fit0_sae <- gbm(Y ~ ., data = data.frame(Y = traindatUC$T_risk_treated_sae[traindatUC$INTENSIVE==0] - traindatUC$sae[traindatUC$INTENSIVE==0],
#                                          traindatUC[traindatUC$INTENSIVE==0,vars]),
#                 distribution = "gaussian",
#                 weights = traindatUC$sae_ipcw[traindatUC$INTENSIVE==0],
#                 shrinkage = 0.01, 
#                 n.trees = 1000,   
#                 interaction.depth = 2, 
#                 n.minobsinnode = 1, 
...
#                 cv.folds = 5,
#                 n.cores = 5) 
# best_iter <- gbm.perf(fit0_sae, plot.it = T, method = 'cv')                
# tau0 <- predict(fit0_sae, accord_test[,vars], n.trees = best_iter, type="response")
# #sae0_imp <- fit0_sae$variable.importance
# XL_RD_sae <- (tau1+tau0)/2
