...
### Using an grf (regression_forest) + IPCW to estimate the CATE function
# X-learner 10-fold CV estimates
...
for (i in 1:nfold){
  ...
  fit1 <- regression_forest(X = traindatUC[traindatUC$INTENSIVE==1,vars], 
                            Y = traindatUC$cvd[traindatUC$INTENSIVE==1] - traindatUC$T_risk_control_cvd[traindatUC$INTENSIVE==1],
                            #num.trees = 2000, mtry = 6, min.node.size = 5, 
                            sample.weights = traindatUC$cvd_ipcw[traindatUC$INTENSIVE==1], seed=1)
  tau1 <- predict(fit1, testdat[,vars])$predictions
  cvd1_imp[,i] <- variable_importance(fit1)
  
  fit0 <- regression_forest(X = traindatUC[traindatUC$INTENSIVE==0,vars], 
                            Y = traindatUC$T_risk_treated_cvd[traindatUC$INTENSIVE==0] - traindatUC$cvd[traindatUC$INTENSIVE==0],
                            #num.trees = 2000, mtry = 6, min.node.size = 5,
                            sample.weights = traindatUC$cvd_ipcw[traindatUC$INTENSIVE==0], seed=1)
  tau0 <- predict(fit0, testdat[,vars])$predictions
  cvd0_imp[,i] <- variable_importance(fit0)
  ...
  fit1 <- regression_forest(X = traindatUC[traindatUC$INTENSIVE==1,vars], 
                            Y = traindatUC$cvd[traindatUC$INTENSIVE==1] - traindatUC$T_risk_control_cvd[traindatUC$INTENSIVE==1],
                            #num.trees = 1000, mtry = 6, min.node.size = 1, 
                            sample.weights = traindatUC$cvd_ipcw[traindatUC$INTENSIVE==1], seed=1)
  tau1 <- predict(fit1, testdat[,vars])$predictions
  cvd1_imp[,i] <- variable_importance(fit1)
...
plot(XL_RD_cvd, sum_cvd$XL_RD_cvd)


########################################################################################################################
traindatUC <- tmpdata[tmpdata$uncensored_cvd==1, ]
traindatUC$cvd <- ifelse(traindatUC$t_cvds>t50, 0, traindatUC$cvd)
fit1_cvd <- regression_forest(X = traindatUC[traindatUC$INTENSIVE==1,vars], 
                              Y = traindatUC$cvd[traindatUC$INTENSIVE==1] - traindatUC$T_risk_control_cvd[traindatUC$INTENSIVE==1],
...
fit0_cvd <- regression_forest(X = traindatUC[traindatUC$INTENSIVE==0,vars],
                              Y = traindatUC$T_risk_treated_cvd[traindatUC$INTENSIVE==0] - traindatUC$cvd[traindatUC$INTENSIVE==0],
                              #num.trees = 1000,  min.node.size = 1, 
                              sample.weights = traindatUC$cvd_ipcw[traindatUC$INTENSIVE==0], seed=1)
...
fit1_sae <- ranger(Y ~ ., data = data.frame(Y = traindatUC$sae[traindatUC$INTENSIVE==1] - traindatUC$T_risk_control_sae[traindatUC$INTENSIVE==1],
                   ...
                   keep.inbag = TRUE, num.trees = 1000, mtry = 6, min.node.size = 5,
