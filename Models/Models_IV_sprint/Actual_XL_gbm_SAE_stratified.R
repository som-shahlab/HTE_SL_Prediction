...
# Sgbm_risk_treated_sae <- rep(NA,dim(tmpdata_sae)[1])
# Sgbm_risk_control_sae <- rep(NA,dim(tmpdata_sae)[1])
# Tgbm_risk_treated_sae <- rep(NA,dim(tmpdata_sae)[1])
# Tgbm_risk_control_sae <- rep(NA,dim(tmpdata_sae)[1])
...
for (l in 1:length(maxdepths)){
  ...
  for (j in 1:length(minobs)){
    ...
    for (k in 1:length(subsamples)){
      ...
      #set.seed(1)
      gbm_tune_treated <- gbm(Surv(t_saes,sae)~., data=traindat1,
                              distribution = "coxph",
                              shrinkage = 0.005,
                              n.trees = 3000,
...
set.seed(1)
gbm_final_treated <- gbm(Surv(t_saes,sae)~., data=traindat1,
                         distribution = "coxph",
                         shrinkage = 0.001,
                         n.trees = 10000,
...
basehaz.cum1 <- basehaz.gbm(traindat1$t_saes, traindat1$sae, pred.train1, t.eval = t50, cumulative = TRUE)
...
for (l in 1:length(maxdepths)){
  ...
  for (j in 1:length(minobs)){
    ...
    for (k in 1:length(subsamples)){
      bag.fraction <- subsamples[k]
      # set.seed(1)
      gbm_tune_control <- gbm(Surv(t_saes,sae)~., data=traindat0,
                              distribution = "coxph",
                              shrinkage = 0.005,
                              n.trees = 3000,
                              interaction.depth = interaction.depth,
...
set.seed(1)
gbm_final_control <- gbm(Surv(t_saes,sae)~., data=traindat0,
                         distribution = "coxph",
                         shrinkage = 0.001,
                         n.trees = 10000,
...
# Estimate the cumulative baseline hazard function using training data
basehaz.cum0 <- basehaz.gbm(traindat0$t_saes, traindat0$sae, pred.train0, t.eval = t50, cumulative = TRUE)
...
for (l in 1:length(maxdepths)){
  ...
  for (j in 1:length(minobs)){
    ...
    for (k in 1:length(subsamples)){
      bag.fraction <- subsamples[k]
      # set.seed(1)
      gbmfit2 <- gbm(Surv(t_saes,sae)~., data=traindat,
                     distribution = "coxph",
                     shrinkage = 0.005,
                     n.trees = 10000,
                     interaction.depth = interaction.depth,
...
set.seed(1)
gbmfit_final <- gbm(Surv(t_saes,sae)~., data=traindat, 
                    distribution = "coxph",
                    shrinkage = 0.001, 
                    n.trees = 20000,   
...
# Estimate the cumulative baseline hazard function using training data
basehaz.cum <- basehaz.gbm(traindat$t_saes, traindat$sae, pred.train, t.eval = t50, cumulative = TRUE)
...
# save results 
risk_gbm_sae <- data.frame(testIndexes, Sgbm_risk_control_sae, Sgbm_risk_treated_sae, Tgbm_risk_control_sae, Tgbm_risk_treated_sae)
write.csv(risk_gbm_sae, paste0("./sae_tuned_results/stratified_split_gbm_sae",i,".csv"), row.names = F)

# best_params <- data.frame(t(best_params_control), t(best_params_treated), t(best_params_S))
# write.csv(best_params, paste0("./sae_tuned_results/gbm_best_params_sae",i,".csv"), row.names = F)
